...
def parse_time_to_minutes(t):
    """
    Абсолютно строгая функция извлечения времени.
    Пропускает только чистые числа или явные форматы времени (часы/минуты).
    """
    t_str = str(t).lower().strip().replace(',', '.')
    
    # Сразу отрезаем технические суффиксы дубликатов (например, _1, _2), 
    # которые добавляет наша функция deduplicate при парсинге.
    t_str = re.sub(r'_\d+$', '', t_str).strip()
    
    # Ищем часы и минуты с жесткими границами слова (чтобы не путать с другими словами)
    h_match = re.search(r'(\d+[.]?\d*)\s*(h|ч)(?:\b|\s|$)', t_str)
    m_match = re.search(r'(\d+[.]?\d*)\s*(m|min|мин)(?:\b|\s|$)', t_str)
    
    # Если нашли часы или минуты — считаем
    if h_match or m_match:
        h = float(h_match.group(1)) if h_match else 0.0
        m = float(m_match.group(1)) if m_match else 0.0
        return h * 60.0 + m
        
    # Если букв h/m нет, проверяем, что это ЧИСТОЕ число (например '20')
    if re.fullmatch(r'\d+[.]?\d*', t_str):
        return float(t_str)
        
    # Все остальное (любые тексты, включая "Blank corrected 600") жестко отбрасываем
    return np.nan
