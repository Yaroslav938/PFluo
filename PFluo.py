...
def clean_number_str(val):
    ...
    # Убираем пробелы, знаки больше/меньше и странные минусы
    val = re.sub(r'\s+', '', val)
    val = val.replace('<', '').replace('>', '').replace('−', '-')
    
    # Обработка тысяч и десятичных знаков (US vs EU форматы)
    if ',' in val and '.' in val:
        if val.rfind(',') > val.rfind('.'):
            val = val.replace('.', '').replace(',', '.') # EU: 1.234,56 -> 1234.56
        else:
            val = val.replace(',', '') # US: 1,234.56 -> 1234.56
    else:
        val = val.replace(',', '.') # Обычная русская запятая
...
def process_dataframe(df, sample_col, time_cols, alpha_grubbs, alpha_norm):
    ...
    for sample_name, group in grouped:
        for time_col in time_cols:
            ...
            # Прогоняем колонку через бронебойный очиститель чисел
...
def parse_uploaded_file(uploaded_file, key_prefix):
    """Мощнейший сканер блоков данных."""
    ...
    if well_row_idx is not None:
        ...
        # Сканируем время только СЛЕВА от данных, чтобы не спутать цифры
        time_rows = []
        time_col_for_row = {}
        ...
        # СТРОГОЕ разбиение на блоки
        blocks = []
        current_block = [time_rows[0]]
        for i in range(1, len(time_rows)):
            if time_rows[i] - time_rows[i-1] > 1:
        ...
        # Интеллектуальный отбор таблицы
        for block in blocks:
            ...
            if "average" in title or "deviation" in title or "standard" in title:
                score -= 10000000
...
def render_analysis_ui(file_obj, title, prefix, alpha_grubbs, alpha_norm):
    ...
    if is_raw:
        st.success("✅ Сырой файл FLUOstar отсканирован (выбрана корректная таблица, мусор отброшен)!")
    else:
    ...
    if st.button(f"Рассчитать статистику {title}") and sample_col and time_cols:
        with st.spinner('Анализ данных...'):
            ...
            # Для правильного объединения сразу добавляем числовое время
        ...
        with tab1:
            ...
            if not res_df.empty:
                ...
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    res_df.drop(columns=['Время_num'], errors='ignore').to_excel(writer, index=False, sheet_name=f'{prefix}_Results')
        ...
        with tab2:
            ...
            if plot_df.empty:
                ...
                if plot_df.empty:
                    ...
                    # ДОБАВЛЕНО: Интерактивный фильтр образцов над графиком
                    all_samples = plot_df['Образец'].unique()
                    selected_samples = st.multiselect(
                        f"Отображаемые образцы ({prefix}):",
                        options=all_samples,
                        default=all_samples,
                    ...
                    if filtered_plot_df.empty:
                        st.info("Пожалуйста, выберите хотя бы один образец для отображения на графике.")
                    else:
                        err_col = 'Ст. ошибка (SE)' if filtered_plot_df['Ст. ошибка (SE)'].sum() > 0 else None
                        
                        # Расширяем цветовую палитру для множества лунок (Alphabet содержит 26 контрастных цветов)
                        ...
                        # Увеличиваем высоту графика, чтобы линии и легенда не сжимались
...
if 'res_lumi' in st.session_state and 'res_od' in st.session_state:
    ...
    # Теперь сводим данные по АБСОЛЮТНОМУ ЧИСЛОВОМУ ВРЕМЕНИ
    res_lumi_merged = st.session_state['res_lumi'].dropna(subset=['Время_num'])
    res_od_merged = st.session_state['res_od'].dropna(subset=['Время_num'])
    
    merged = pd.merge(res_lumi_merged, res_od_merged, on=['Образец', 'Время_num'], suffixes=('_Lumi', '_OD'))
    
    if merged.empty:
        st.warning("⚠️ Внимание: Не найдено совпадающих образцов и временных точек (в минутах) между Lumi и OD600. Расчет отношения невозможен.")
    else:
        ...
        # Оставляем только нужные колонки для показа
        show_cols = ['Образец', 'Время_Lumi', 'Среднее_Lumi', 'Среднее_OD', 'Lumi / OD600']
        ...
        if merged.empty:
            ...
            # ДОБАВЛЕНО: Интерактивный фильтр образцов для сводного графика
            all_samples_ratio = merged['Образец'].unique()
            selected_samples_ratio = st.multiselect(
                "Отображаемые образцы (Lumi/OD600):",
            ...
            if filtered_ratio.empty:
                ...
                fig3 = px.line(filtered_ratio, x='Время_num', y='Lumi / OD600', color='Образец', markers=True, 
                               title="Отношение Luminescence к OD600",
                               color_discrete_sequence=px.colors.qualitative.Alphabet * 5)
                fig3.update_layout(xaxis_title="Время (в минутах / циклах)", yaxis_title="Отношение Lumi / OD", height=700)
        ...
        with pd.ExcelWriter(output_merged, engine='openpyxl') as writer:
            merged[show_cols].rename(columns={'Время_Lumi': 'Время'}).to_excel(writer, index=False, sheet_name='Lumi_OD_Ratio')
