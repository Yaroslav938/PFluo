...
def parse_uploaded_file(uploaded_file, key_prefix):
    ...
    if well_row_idx is not None:
        ...
        # СТРОГОЕ разбиение на блоки (любой разрыв больше 1 строки создает новую таблицу)
        blocks = []
        ...
        # Интеллектуальный отбор таблицы (ищем Blank corrected или Raw Data)
        for block in blocks:
...
if 'res_lumi' in st.session_state and 'res_od' in st.session_state:
    ...
    # Теперь сводим данные по АБСОЛЮТНОМУ ЧИСЛОВОМУ ВРЕМЕНИ, чтобы избежать любых нестыковок текста
