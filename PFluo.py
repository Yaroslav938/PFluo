...
def parse_uploaded_file(uploaded_file, key_prefix):
    ...
    if well_row_idx is not None:
        ...
        # Сканируем время: игнорируем любые строки, содержащие слова-паразиты
        time_rows = []
        time_col_for_row = {}
        ...
        # УЛУЧШЕННОЕ РАЗБИЕНИЕ НА БЛОКИ (Учитывает и разрывы строк, и сброс времени назад к 0)
        blocks = []
        ...
        for i in range(1, len(time_rows)):
            ...
            if row_idx - prev_row_idx > 1:
                ...
                # Время сбросилось или повторилось (началась новая таблица)
                blocks.append(current_block)
                current_block = [row_idx]
            else:
