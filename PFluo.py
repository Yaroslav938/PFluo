    ...
    if f'res_{prefix}' in st.session_state:
        ...
        with tab2:
            ...
            def extract_time_val(t):
                ...
                # Жесткая защита: игнорируем технические колонки
            ...
            plot_df = plot_df.dropna(subset=['Время_num']) # Удаляем мусорные точки с оси X
            plot_df = plot_df.sort_values(by=['Образец', 'Время_num'])
            
            # ПАТЧ: Plotly отказывается рисовать точки, если ошибка (SE) равна NaN (как бывает при N=1).
            # Мы заполняем NaN нулями ТОЛЬКО для графика, в таблице останется NaN.
            plot_df['Ст. ошибка (SE)'] = plot_df['Ст. ошибка (SE)'].fillna(0)

            fig = px.line(plot_df, x='Время_num', y='Среднее', color='Образец', markers=True, 
                          error_y='Ст. ошибка (SE)', title=f"Динамика {title}")
...
if 'res_lumi' in st.session_state and 'res_od' in st.session_state:
    ...
    if merged.empty:
        ...
        merged['Время_num'] = merged['Время'].apply(extract_time_val)
        merged = merged.dropna(subset=['Время_num']) # Зачищаем ось X
