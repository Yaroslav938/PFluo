...
def process_dataframe(df, sample_col, time_cols, alpha_grubbs, alpha_norm):
    ...
    for sample_name, group in grouped:
        for time_col in time_cols:
            ...
            # Прогоняем колонку через бронебойный очиститель чисел (поточечно)
...
def parse_uploaded_file(uploaded_file, key_prefix):
    ...
    if well_row_idx is not None:
        ...
        # Сканируем время: собираем все строки, похожие на время
        time_rows = []
        time_col_for_row = {}
        ...
        # СТРОГОЕ разбиение на блоки (изолируем таблицы друг от друга)
        blocks = []
        ...
        for i in range(1, len(time_rows)):
            ...
            if row_idx - prev_row_idx > 1:
                ...
                # Время сбросилось (началась новая таблица от прибора)
                blocks.append(current_block)
                current_block = [row_idx]
            else:
        ...
        # Выбираем ТОЛЬКО ОДИН лучший блок
        for block in blocks:
...
def render_analysis_ui(file_obj, title, prefix, alpha_grubbs, alpha_norm):
    ...
    if is_raw:
        st.success("✅ Сырой файл FLUOstar отсканирован (выбрана строго 1 таблица без дублей времени)!")
    else:
    ...
    with col2:
        if is_raw:
            # Выбираем по умолчанию только те колонки, которые являются нормальным временем
            ...
            if plot_df.empty:
                ...
                if plot_df.empty:
                    ...
                    if filtered_plot_df.empty:
                        ...
                        # Расширенная контрастная палитра для клонов
...
if 'res_lumi' in st.session_state and 'res_od' in st.session_state:
    ...
    # Безопасное объединение через округленные минуты
    ...
    if merged.empty:
        ...
        merged = merged.dropna(subset=['Время_num_round', 'Lumi / OD600'])
        ...
        if merged.empty:
            ...
            if filtered_ratio.empty:
                ...
                fig3 = px.line(filtered_ratio, x='Время_num_round', y='Lumi / OD600', color='Образец', markers=True, 
