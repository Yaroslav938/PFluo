...
def parse_time_to_minutes(t):
    """Умное извлечение числового времени из названия колонки"""
    t_str = str(t).lower().strip().replace(',', '.')
    
    # Точечная защита от технического мусора (строгое совпадение)
    if t_str in ['od600', 'образец', 'sample', 'well', 'content']:
        return np.nan
        
    # Ищем часы и минуты (включая русские 'ч' и 'мин')
    h_match = re.search(r'(\d+[.]?\d*)\s*(h|ч)', t_str)
    m_match = re.search(r'(\d+[.]?\d*)\s*(m|мин)', t_str)
    ...
    # Если это просто число (цикл или минута без подписи)
    match = re.search(r'(\d+[.]?\d*)', t_str)
    if match:
        # Предотвращаем баг, когда колонка называется "OD 600 nm", "600nm" 
        if 'nm' in t_str or 'od' in t_str:
            return np.nan
