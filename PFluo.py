...
def parse_time_to_minutes(t):
    """
    Абсолютно строгая и при этом универсальная функция извлечения времени.
    """
    t_str = str(t).lower().strip().replace(',', '.')
    
    # Сразу отрезаем технические суффиксы дубликатов (например, _1, _2)
    t_str = re.sub(r'_\d+$', '', t_str).strip()
    
    # 1. Защита от системных колонок (исключаем слова, которые точно не являются временем)
    if any(w in t_str for w in ['od600', 'образец', 'sample', 'unnamed', 'blank', 'raw', 'well', 'content', 'deviation', 'average', 'standard']):
        return np.nan
        
    # 2. Поиск явных часов и минут
    ...
    # 3. Универсальный фоллбэк: берем первое число в строке
    # Это спасет, если время записано как "Cycle 1", "10", "1.5" или с невидимыми символами
...
def process_dataframe(df, sample_col, time_cols, alpha_grubbs, alpha_norm):
    ...
    for sample_name, group in grouped:
        for time_col in time_cols:
            # Защита от русских запятых в числах
            raw_series = group[time_col]
            if raw_series.dtype == object:
                raw_series = raw_series.astype(str).str.replace(',', '.')
...
def render_analysis_ui(file_obj, title, prefix, alpha_grubbs, alpha_norm):
    ...
    if f'res_{prefix}' in st.session_state:
        ...
        with tab2:
            ...
            # Приводим к числам (используем нашу новую умную функцию)
            ...
            if plot_df.empty:
                ...
                # ПАТЧ: Если суммарная ошибка равна нулю, Plotly может глючить, поэтому отключаем error_y
...
if 'res_lumi' in st.session_state and 'res_od' in st.session_state:
    ...
    if merged.empty:
        ...
        if merged.empty:
            ...
            with st.expander("Техническая информация (Отладка)"):
                st.write("Колонки времени:", st.session_state['res_lumi']['Время'].unique())
                st.write("Распознанные числа:", st.session_state['res_lumi']['Время'].apply(parse_time_to_minutes).unique())
        else:
