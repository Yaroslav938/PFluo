...
def dixon_test(x, alpha=0.05):
    """Рекурсивный Q-тест Диксона для поиска и удаления выбросов."""
    # Табличные значения Q-критерия Диксона (r10) для малых выборок N от 3 до 30
    q95 = {3: 0.970, 4: 0.829, 5: 0.710, 6: 0.625, 7: 0.568, 8: 0.526, 9: 0.493, 10: 0.466, 
           11: 0.444, 12: 0.426, 13: 0.410, 14: 0.396, 15: 0.384, 16: 0.374, 17: 0.365, 18: 0.356, 
           19: 0.349, 20: 0.342, 21: 0.337, 22: 0.331, 23: 0.326, 24: 0.321, 25: 0.317, 26: 0.312, 
           27: 0.308, 28: 0.304, 29: 0.300, 30: 0.296}
           
    q99 = {3: 0.994, 4: 0.926, 5: 0.821, 6: 0.740, 7: 0.680, 8: 0.634, 9: 0.598, 10: 0.568, 
           11: 0.542, 12: 0.522, 13: 0.503, 14: 0.488, 15: 0.475, 16: 0.463, 17: 0.452, 18: 0.442, 
           19: 0.433, 20: 0.425, 21: 0.418, 22: 0.411, 23: 0.404, 24: 0.399, 25: 0.393, 26: 0.388, 
           27: 0.384, 28: 0.380, 29: 0.376, 30: 0.372}
    ...
    while True:
        n = len(x)
        # Тест Диксона обычно применяется для малых выборок
        if n < 3 or n > 30:
            break
            
        sorted_x = np.sort(x)
        gap_up = sorted_x[-1] - sorted_x[-2]
        gap_down = sorted_x[1] - sorted_x[0]
        range_x = sorted_x[-1] - sorted_x[0]
        
        if range_x == 0:
            break
            
        q_stat_up = gap_up / range_x
        q_stat_down = gap_down / range_x
        
        q_stat = max(q_stat_up, q_stat_down)
        
        q_dict = q99 if alpha <= 0.01 else q95
        q_crit = q_dict.get(n, 1.0)
        
        if q_stat > q_crit:
            if q_stat_up > q_stat_down:
                outliers.append(sorted_x[-1])
                x = np.delete(sorted_x, -1)
            else:
                outliers.append(sorted_x[0])
                x = np.delete(sorted_x, 0)
        else:
            break
...
def process_dataframe(df, sample_col, time_cols, outlier_method, alpha_outlier, alpha_norm):
    ...
    for sample_name, group in grouped:
        for time_col in time_cols:
            ...
            # Выбор метода поиска выбросов
            if outlier_method == "Тест Граббса":
                clean_values, outliers = grubbs_test(raw_values, alpha=alpha_outlier)
            else:
                clean_values, outliers = dixon_test(raw_values, alpha=alpha_outlier)
...
def render_analysis_ui(file_obj, title, prefix, outlier_method, alpha_outlier, alpha_norm):
    st.header(title)
    ...
    if st.button(f"Рассчитать статистику {title}") and sample_col and time_cols:
        with st.spinner('Анализ данных...'):
            res_df, clean_long_df = process_dataframe(df_raw, sample_col, time_cols, outlier_method, alpha_outlier, alpha_norm)
        ...
        with tab3:
            ...
            if not res_df.empty:
                ...
                if len(subset) > 2:
                    ...
                    st.info(f"**Метод поиска выбросов:** {outlier_method} | **Тест норм.:** {stat_row['Тест нормальности']} | **p-value:** {stat_row['p-value (норм.)']:.4f} | **Нормально?** {stat_row['Распределение нормально?']}")
                else:
...
st.sidebar.markdown("---")
outlier_method = st.sidebar.selectbox("Метод поиска выбросов", ["Тест Граббса", "Q-тест Диксона"])
alpha_outlier = st.sidebar.slider("Уровень значимости (α) для выбросов", 0.01, 0.10, 0.05, 0.01)
alpha_norm = st.sidebar.slider("Уровень значимости (α) теста нормальности", 0.01, 0.10, 0.05, 0.01)

# БЛОК 1: LUMI
if file_lumi is not None:
    render_analysis_ui(file_lumi, "Luminescence", "lumi", outlier_method, alpha_outlier, alpha_norm)
else:
...
if file_od is not None:
    st.markdown("---")
    render_analysis_ui(file_od, "OD600", "od", outlier_method, alpha_outlier, alpha_norm)
