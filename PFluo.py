...
def parse_uploaded_file(uploaded_file, key_prefix):
    """Безопасный сканер данных с системой отката (Fallback)."""
    ...
    def _fallback(df_in):
        """Если авто-сканер не сработал, возвращаем таблицу как есть."""
        if df_in.empty: 
    ...
    # Автоопределение разделителей для CSV файлов
    ...
    if well_row_idx is not None:
        ...
        for i in range(len(df)):
            for c in range(start_col + 1): # Ищем время только в левой части
        ...
        if not time_rows:
            return _fallback(df) # Переход на ручной режим
        ...
        for block in blocks:
            ...
            if "blank corrected" in title:
                ...
                score -= num_count * 0.9 # Понижаем приоритет, но не удаляем блок полностью
        ...
        return _fallback(df) # Если матрица лунок не найдена - ручной режим
...
def render_analysis_ui(file_obj, title, prefix, alpha_grubbs, alpha_norm):
    ...
    # Жесткая защита от ошибки отрисовки пустых таблиц
    ...
    if not cols:
        ...
        # Показываем сырой файл для отладки
        try:
    ...
    if is_raw:
        ...
        st.info("ℹ️ Файл загружен в ручном режиме (стандартная таблица). Пожалуйста, выберите нужные колонки ниже.")
    ...
    with col2:
        ...
        # Защита от отсутствия временных колонок
        safe_defaults = [c for c in default_times if c in cols]
        time_cols = st.multiselect(f"Колонки со временем ({prefix})", [c for c in cols if c != sample_col], default=safe_defaults, key=f"{prefix}_time")
        ...
        with tab4:
            ...
            if st.button("Сравнить (T-test)", key=f"ttest_btn_{prefix}"):
                ...
                if len(valA) < 2 or len(valB) < 2:
                    ...
                    t_stat, p_val = stats.ttest_ind(valA, valB, equal_var=False)
